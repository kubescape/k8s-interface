...
func GenerateInstanceID(w workloadinterface.IWorkload) ([]InstanceID, error) {
	...
	return listInstanceIDs(ownerReferences, ephemeralContainers, w.GetApiVersion(), w.GetNamespace(), w.GetKind(), w.GetName())
}

// GenerateInstanceIDFromPod generates instance ID from pod
func GenerateInstanceIDFromPod(pod *core1.Pod) ([]InstanceID, error) {
	return listInstanceIDs(pod.GetOwnerReferences(), pod.Spec.EphemeralContainers, pod.APIVersion, pod.GetNamespace(), pod.Kind, pod.GetName())
}
...
func GenerateInstanceIDFromString(input string) (*InstanceID, error) {
	...
	instanceID.ephemeralContainerName = strings.TrimPrefix(fields[4+i], prefixEphemeralContainer)
